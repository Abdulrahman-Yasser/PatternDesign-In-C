...
void HistogramDisplay_getValue(HistogramDisplay* const me){
    ...
    me->my_lcd->LCD_Write_Cmd(LCD_I2C_SET_DDRAM_ADDRESS(0, 0));

    s = int_to_string(tmd.time.hours);
    me->my_lcd->LCD_Write_Data(s);
    ...
    me->my_lcd->LCD_Write_Cmd(LCD_I2C_INST_CURSOR_GO_2ND_LINE);

    me->my_lcd->LCD_Write_Data("Temp : ");
